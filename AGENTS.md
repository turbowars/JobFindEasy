...
- Profile facts live in `src/resume/profile.py`. The LLM prompt references them, the dashboard's "Apply with Codex" prompt references them, future autofill tooling references them. **Never restate.**
...
Use [`AskUserQuestion`](.Codex/) for choice-driven decisions. Use plain text for "I'm seeing X, here's why I think Y, want me to proceed?" framing.

"""LLM prompt + user-message builder for the EM cover letter.
...
- Use plural team voice. Mix in "my team", "the engineers I led", "we shipped". Avoid solo "I architected/built/designed" phrasing — those are IC-coded verbs that undersell the management frame.
...
def _signal_table_block() -> str:
    ...
    for key, entry in profile.COVER_LETTER_BULLETS_BY_SIGNAL.items():
...
def build_user_message(
    ...
    parts = [
        ...
        f"FRAME: {frame}  (renderer uses {'hybrid 40/60 opening' if frame == 'hybrid' else 'standard EM opening'})",
    ]
    if referrer_name:
